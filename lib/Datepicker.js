class DatepickerScript {
  ...
  constructor (options = {}) {
    ...
    this._utilInternal = new DatepickerScriptUtils(this._date)
  }
  ...
  computePanel (month = this.getMonth, year = this.getFullYear) {
    ...
    const monthsDays = this._utilInternal.getMonthDays(this._date)
    const getDayStringName = this._utilInternal.getDayStrName(getUTCFirstInMonth)


    if (getDayStringName !== 'mon') {
      const minDays = this._utilInternal.getEnumDays(getDayStringName)
      ...
      console.log(getLastMonthIndex)
      const len = getDaysInMonthBefore - minDays
      for (let i = getDaysInMonthBefore; i > len; i--) this.data.unshift(this.setDayInstanceItem(getLastMonthIndex, i))
    }
