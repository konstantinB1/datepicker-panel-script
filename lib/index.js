...
class DatepickerPanelScriptDay {
  ...
  constructor (dateObj, listener) {
    ...
    this.isToday = (dateObj.toDateString() === compDate.toDateString())
    this.curMonth = (dateObj.getMonth() === compDate.getMonth())
...
class DatepickerPanelScript {
   ...
   */
  constructor (dateObj = null, options = {}) {
    this._date = null
    this.data = []
   ...
   * @param  {Number} m 
   * @param  {Number} d 
   * @return {DatepickerPanelScriptDay}
   */
  setDayInstanceItem (m, d) {
    const utcDayDate = new Date(Date.UTC(this.getFullYear, m, d))
    return new DatepickerPanelScriptDay(utcDayDate, this.listener)
  }
  ...
  clear () {
    this.data.forEach(dInst => {
      if (dInst && dInst instanceof DatepickerPanelScriptDay && dInst._listener) {
  ...
  computePanel (month = this.getMonth, year = this.getFullYear) {
    ...
    if (getDayStringName !== 'mon') {
      const minDays = DAYS_ENUM[getDayStringName]
